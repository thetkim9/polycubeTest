...
- 실행: ./gradlew bootRun
- 테스트: ./gradlew test
- 빌드 시 테스트도 같이 이루어지며, 평균적으로 1~2분 소요
- 실행 시 평균적으로 10~20초 소요되며, Spring Batch의 Step: [checkWinnersStep]이 [COMPLETED] 되었다고 출력되면, 준비 완료
- 디비는 인메모리에서 작동


## 프로젝트 소감
- Spring Batch를 처음 사용하면서 시간을 많이 잡아먹었지만 정말 즐거웠습니다.
- 스프링을 접한지 얼마 안되기도 했고, 제대로 공부해본적이 없었는데, 이번 프로젝트를 진행하면서 기본기를 다지게 된 것 같아 회사에 감사한 마음이 듭니다.
